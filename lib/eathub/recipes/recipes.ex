defmodule Eathub.Recipes do
  ...
  def list_recipes do
    Repo.all(Recipe)
    |> Enum.map(fn(recipe) -> load_user(recipe) end)
    |> Enum.map(fn(recipe) -> load_steps(recipe) end)
    |> Enum.map(fn(recipe) -> load_ingredients(recipe) end)
  end
  ...
  def load_steps(recipe) do
    Repo.preload(recipe, :steps)
  end

  def load_ingredients(recipe) do
